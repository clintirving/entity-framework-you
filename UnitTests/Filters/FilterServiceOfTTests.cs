...
namespace EfYouTests.Filters
{
    ...
    public class FilterServiceTests
    {
        ...
        [TestMethod]
        public void FilterResultsOnGet_CallsFilterResultsOnIds()
        {
            // Arrange
        ...
        [TestMethod]
        public void FilterResultsOnGet_PrimaryKeyOfTypeIsInt_ReturnsQueryableFilteredByIds()
        {
            // Arrange
            var filterService = GetFilterServiceMock();
            filterService.Setup(x => x.FilterResultsOnIds(It.IsAny<IQueryable<DummyEntity>>(), It.IsAny<List<long>>()))
                .Returns<IQueryable<DummyEntity>, List<long>>((x, y) => x.Where(z => z.Id == 2));
        ...
        [TestMethod]
        public void FilterResultsOnGet_PrimaryKeyOfTypeIsLong_ReturnsQueryableFilteredByIds()
        {
            // Arrange
        ...
        public void AddAggregationFilter_FourDummyObjectsWithOneGroupBy_ReturnsQueryableWithCorrectExpressionTree()
        {
            ...
            Assert.IsTrue(results.ToString()
                .Contains(
                    ".GroupBy(x => new CustomType(Choices = x.Choices), x => Convert(x.Id)).GroupBy(x => x.Max(), x => x.OrderByDescending(y => y).ToList()).OrderByDescending(x => x.Key).Skip(0).Take(20)"));
        }
        ...
        public void AddAggregationFilter_FourDummyObjectsWithTwoGroupBys_ReturnsQueryableWithCorrectExpressionTree()
        {
            ...
            Assert.IsTrue(results.ToString()
                .Contains(
                    ".GroupBy(x => new CustomType(Choices = x.Choices, FilterableInt = x.FilterableInt), x => Convert(x.Id)).GroupBy(x => x.Max(), x => x.OrderByDescending(y => y).ToList()).OrderByDescending(x => x.Key).Skip(12).Take(12)"));
        }
    }
}